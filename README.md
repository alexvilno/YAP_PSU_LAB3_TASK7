...
```
В матрице M1 с помощью метода ```public Matrix GetMaxTrippleSubMatrix();``` найдем максимальную(наибольшая сумма элементов) подматрицу размера 3 х 3: 

```c#
Console.WriteLine("M1 наибольшая подматрица (сумма эл-тов) = " + M1.max_sum_triple_matrix());
Matrix Sub = M1.GetMaxTrippleSubMatrix();
Console.WriteLine("Большая подматрица M1: " + Sub.ToString());
```
*Вывод:*
